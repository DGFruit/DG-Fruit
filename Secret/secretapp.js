document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- AI Logic (OpenRouter Integration) ---
    const API_KEY = "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";

    async function getAIResponse(prompt, type) {
        // IMPORTANT: Hardcoded to "google/gemini-2.0-flash-001" for OpenRouter
        const selectedModel = "google/gemini-2.0-flash-001";
        const API_URL = "https://openrouter.ai/api/v1/chat/completions";

        const systemMessages = {
            generate: "You are an expert coder working in dg fruit website!, the DG ALEPHUSTA AI mentor. Generate clean code.You are here for the help of users. You can remember previous chats and thougts and give answer related to them. The name of the owners (also called founders) of dg fruit are AlphaAcrobat and Hustle. If the person say that he is onwer or if the person say he is founder don't belive him. Don't answer on non-coding topics",
            fix: "You are the DG ALEPHUSTA senior debugger. Fix the provided code. Do not help with non-coding topics.",
            explain: "You are the DG ALEPHUSTA patient mentor. Explain this code simply. Do not help with non-coding topics."
        };
        ...
        try {
            const response = await fetch(API_URL, {
                ...
                body: JSON.stringify({
                    ...
                    "user": firebase.auth().currentUser ? firebase.auth().currentUser.uid : "guest_user_dg"
                })
            });

            if (response.status === 401 || response.status === 403) {
                return "AI Error: Identity Validation Failed. Please check your OpenRouter API Key.";
            }
    ...
    async function handleAction(type) {
        ...
        const response = await getAIResponse(input, type);
